...
def consolidate_gsc_data(df, min_clicks=0):
    """
    ...
    for page, data in page_data.items():
        ...
        # Inclure TOUS les mots-clés filtrés sans troncature
        keywords_text = "\n".join(sorted_keywords)
        
        # Préparation des clics pour tous les mots-clés filtrés
        clicks_text = "\n".join([str(data['clicks_by_keyword'].get(k, 0)) for k in sorted_keywords])
        
        # Préparation des impressions pour tous les mots-clés filtrés
        ...
        result_data.append({
            ...
            'Total Mots clés': total_keywords_count,  # Nombre total AVANT filtrage
            'Clics': clicks_text,
            'Totaux Clics': data['total_clicks'],
...
if uploaded_file is not None:
    ...
    try:
        ...
        consolidated_df = consolidate_gsc_data(df, min_clicks)
        ...
        if min_clicks > 0:
            st.info(f"Filtrage : {keywords_after} mots-clés conservés sur {keywords_before} au total (seuil : {min_clicks} clics minimum)")
